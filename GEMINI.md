...
- The user's operating system is Windows (win32), and the preferred shell is PowerShell. Shell commands should use PowerShell syntax (e.g., `Remove-Item`, `Get-ChildItem`, `$env:VAR`, etc.). Validate any Python environments using `uv run` or ensuring `.venv` is correct.
...
## Project Conventions
- **Secrets Management**: All secrets or configuration details (Tokens, MySQL credentials, Warhorn tokens) are kept exclusively in the `.env` file and loaded using `python-dotenv`.
- **Modularity**: Currently migrating from a single `bot.py` monolithic script to a modular `cogs/` directory architecture (Discord.py standard practice).
